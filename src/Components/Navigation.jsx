  ...
  return (
    <header className="fixed top-0 w-full left-0 z-50">
        <nav className='flex justify-between items-center container mx-auto py-4 '>
            {
                logo.map((item, index) => (
                  <a href="/" key={index} className="flex gap-2 items-center">
            ...
            <ul className='flex gap-4'>
                {
                    primaryLinks.map((links, index) => (
                        <li key={index}>
                            <a href={links.url || "#"} className={`text-white ${elementOnHover ? "text-sm pb-0 transition-all hover:pb-2 hover:border-b-2 hover:border-orange-500" : "text-sm pb-0"}`}>
                                {links.text}
                            </a>
                        </li>
                    ))
            ...
            <ul className='flex gap-3'>
              {
                 secondaryLinks.map((links, index) => (
                  <li key={index}>
                    <a href={links.url || "#"} className={`text-white ${links.styles || ""}`}>{links.text}</a>
                  </li>
                 ))
              }
            </ul>
